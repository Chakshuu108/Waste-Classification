...
# .fb-box {
#     background: linear-gradient(135deg, #1a2744, #0d1a33);
#     border: 2px solid #2a4080; border-radius: 18px;
#     padding: 1.8rem 2rem; text-align: center;
#     box-shadow: 0 12px 48px rgba(0,0,0,.6);
#     min-width: 280px;
# }
# .fb-box .fb-icon { font-size: 2.4rem; margin-bottom: .4rem; }
# .fb-box .fb-q {
#     font-size: 1rem; font-weight: 600; color: #e8edf5;
#     margin-bottom: 1.2rem; line-height: 1.4;
# }
# .fb-btns { display: flex; gap: .8rem; justify-content: center; }
# .fb-btn {
...
# .fb-yes { background: #2ECC71; color: #0a1a0a; }
# .fb-no  { background: #E74C3C; color: #fff; }
# </style>
# </head>
# <body>
...
#         // Ask user if classification was correct — no action taken either way
#         showFeedbackPopup(result.topClass);
...
# function showFeedbackPopup(topClass) {
#     const color = COLORS[topClass];
#     const overlay = document.createElement('div');
#     overlay.className = 'fb-overlay';
#     overlay.innerHTML = `
#         <div class="fb-box">
#             <div class="fb-icon">${ICONS[topClass]}</div>
#             <div class="fb-q">Was <strong style="color:${color}">${topClass.toUpperCase()}</strong> the correct bin?</div>
#             <div class="fb-btns">
#                 <button class="fb-btn fb-yes" onclick="this.closest('.fb-overlay').remove()">✅ Yes</button>
#                 <button class="fb-btn fb-no"  onclick="this.closest('.fb-overlay').remove()">❌ No</button>
#             </div>
#         </div>`;
...
.fb-bin-btn {
    ...
    gap: .3rem; padding: .7rem .4rem; border-radius: 12px; border: 2px solid;
    background: #0a0e1a; cursor: pointer;
    font-family: 'Space Grotesk', sans-serif; transition: opacity .15s, transform .1s;
    min-height: 72px;
}
.fb-bin-btn:hover { opacity: .8; transform: scale(1.05); }
</style>
</head>
<body>
...
<div style="border-top:1px solid #1e3060;padding-top:.8rem;margin-top:1rem;
     display:flex;justify-content:space-between;font-size:.7rem;color:#4a5a7a;">
    <div>🗑️ VisionBin · Smart Dustbin Waste Classifier</div>
    <div>🪟 Glass &nbsp;|&nbsp; 📄 Paper &nbsp;|&nbsp; ⚙️ Metal &nbsp;|&nbsp; 🌿 Organic &nbsp;|&nbsp; 🧴 Plastic</div>
</div>
    ...
    function renderQ1() {
        overlay.innerHTML = `
            <div class="fb-box">
                ...
                <div class="fb-q">Was <strong style="color:${color}">${topClass.toUpperCase()}</strong> the correct bin?</div>
                <div class="fb-btns">
                    <button class="fb-btn fb-yes" id="fbYes">✅ Yes</button>
                    <button class="fb-btn fb-no"  id="fbNo">❌ No</button>
                </div>
            </div>`;
        overlay.querySelector('#fbYes').onclick = () => overlay.remove();
        overlay.querySelector('#fbNo').onclick  = () => renderQ2();
    }

    function renderQ2() {
        const binBtns = CLASSES.map(cls => `
            <button class="fb-bin-btn" onclick="this.closest('.fb-overlay').remove()"
                style="border-color:${COLORS[cls]};color:${COLORS[cls]};">
                <span style="font-size:1.6rem">${ICONS[cls]}</span>
                <span style="font-size:.8rem;font-weight:700;text-transform:uppercase;">${cls}</span>
            </button>`).join('');
        overlay.innerHTML = `
            <div class="fb-box">
                <div class="fb-q" style="margin-bottom:1rem;">Which was the correct bin?</div>
                <div class="fb-bin-grid">${binBtns}</div>
            </div>`;
    }
